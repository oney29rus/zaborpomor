      ...
      <div className={`flex flex-1 flex-col ${compact ? "p-3" : "p-4 sm:p-5"}`}>
        <h3
          className={`font-bold tracking-tight text-foreground ${
            compact ? "text-base sm:text-lg lg:text-2xl" : "text-xl sm:text-2xl"
          }`}
        >
          <TrackedCityLink
            href={card.href}
        ...
        <p
          className={`mt-1 leading-snug text-muted ${
            compact ? "text-xs line-clamp-2 sm:text-sm lg:text-base" : "text-sm sm:text-base"
          }`}
        >
        ...
        <TrackedCityLink
          ...
          className={`mt-auto inline-flex font-semibold text-accent transition-colors hover:text-accent-hover ${
            compact ? "pt-2 text-xs sm:text-sm lg:text-base" : "pt-3 text-sm sm:text-base"
          }`}
        >
          {card.ctaLabel}
        </TrackedCityLink>
  ...
  return (
    <section className={`scroll-mt-24 bg-[#f5f5f5] ${HOME_SECTION_PY}`}>
      <div className={SECTION_CONTAINER}>
        ...
        <div
          className={`${HOME_SECTION_CONTENT_MT} grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-5`}
        >
        ...
        <div className="mt-4 flex flex-col gap-1.5 sm:mt-8 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-1">
